...
def call_model(system, user):
    ...
    if provider in ("anthropic", "eden") and model:
        model_lower = model.lower()
        if "sonnet" in model_lower or "opus" in model_lower:
            return None, (
                f"BLOCKED: model '{model}' is forbidden. "
                "Only a Haiku-tier model is allowed. "
                "Set AGENTOS_MODEL to a haiku model in .env"
            )
    ...
    if provider == "eden":
        # Routes through Eden AI (Bedrock) instead of a direct Anthropic key -
        # matches how the rest of Hermes is now configured to route Claude
        # calls, so the governor uses the same funded path, not a separate
        # near-empty direct Anthropic key.
        key = os.environ.get("EDEN_AI_KEY")
        if not key or not model:
            return None, "Eden selected but EDEN_AI_KEY or AGENTOS_MODEL is missing."
        try:
            import requests
            resp = requests.post(
            ...
            return None, f"Eden call failed: {e}"
